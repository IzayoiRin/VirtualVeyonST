...
from SwitchTracer import import_from_path
from SwitchTracer.universal.exceptions import SettingErrors, ImportedErrors, SetupErrors
...
class Activator(object):
    ...
    def setup_flask_application(self, role):
        flask_app_factory = st.flask.FlaskApplicationFactory(env=self._environ)
        fapp = flask_app_factory(role)
        flask_app_factory.register_bprinter(fapp, role=role)
        return fapp
